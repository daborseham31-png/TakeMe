...
// Desktop note: only resolveLocationCoordinates (below) uses expo-location,
// and it's never called by anything the admin app reaches (admin's only
// consumer of this file, schools.ts, only needs normalizeLocationText) — the
// try/catch around the one call site already treats a geocoding failure as
// "no coordinates available" and degrades gracefully, so a stub that always
// fails is safe here, and avoids expo-location's native-module dependency
// chain for code this app never really exercises.
const Location = {
  geocodeAsync: async (_address: string): Promise<{ latitude: number; longitude: number }[]> => {
    throw new Error("Geocoding is not available in the TakeMe Admin desktop app.");
  },
};
